...
- **Git используется.** Ветки, коммиты и PR допустимы. Есть CI и CD workflow, но production deploy всё ещё зависит от корректных GitHub secrets и серверного `.env`.
...
- CD уже описан в `.github/workflows/cd.yml`, но перед включением нужно вручную проверить серверный путь, права и production `.env`.
