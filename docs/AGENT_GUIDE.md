...
- **Git используется.** Ветки, коммиты и PR допустимы. Есть CI для тестов, но CD в проекте пока нет.
...
- Нет CD (осознанное решение). CI используется для тестов и smoke-проверок.
